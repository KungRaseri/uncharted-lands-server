...
describe('World Generator', () => {
	...
	describe('generateMap', () => {
		...
		it('should use provided seed', async () => {
			const map1 = await generateMap(
				{ ...defaultMapOptions, seed: 111 },
				defaultNoiseOptions
			);
			
			const map2 = await generateMap(
				{ ...defaultMapOptions, seed: 222 },
				defaultNoiseOptions
			);
		...
		it('should handle different octave values', async () => {
			const lowOctave = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, octaves: 1 }
			);
			
			const highOctave = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, octaves: 8 }
			);
		...
		it('should handle different amplitude values', async () => {
			const result = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, amplitude: 2 }
			);
		...
		it('should handle different frequency values', async () => {
			const result = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, frequency: 2 }
			);
		...
		it('should handle different persistence values', async () => {
			const result = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, persistence: 0.75 }
			);
		...
		it('should handle custom scale function', async () => {
			const result = await generateMap(
				defaultMapOptions,
				{ ...defaultNoiseOptions, scale: (x: number) => x * 2 }
			);
	...
	describe('generateWorldLayers', () => {
		...
		it('should generate unique data for each region', async () => {
			...
			const uniqueCoords = new Set(
				regions.map(r => `${r.xCoord},${r.yCoord}`)
			);
