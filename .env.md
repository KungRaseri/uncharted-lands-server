...
2. **Prisma Schema** - Located in `../client/prisma/schema.prisma`
3. **Environment Variables** - Both need `DATABASE_URL`

### Setup Order
1. Set up client `.env` first (includes database config)
2. Run database migrations from client: `cd ../client && npx prisma migrate dev`
3. Set up server `.env` with same `DATABASE_URL`
