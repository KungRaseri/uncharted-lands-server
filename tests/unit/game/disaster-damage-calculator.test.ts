...
describe('calculateAllDisasterModifiers', () => {
  ...
  describe('No Active Disasters', () => {
    ...
    test('should return all modifiers = 1 when disasters are in WARNING status', () => {
      const disasters = [
        { type: 'DROUGHT', status: 'WARNING', impactEndedAt: undefined },
      ];
    ...
    test('should return all modifiers = 1 when disasters are in RESOLVED status', () => {
      const disasters = [
        { type: 'DROUGHT', status: 'RESOLVED', impactEndedAt: undefined },
      ];
  ...
  describe('Single Disaster - IMPACT Phase', () => {
    ...
    test('LOCUST_SWARM in IMPACT should reduce food to 30% (most severe single resource)', () => {
      const disasters = [
        { type: 'LOCUST_SWARM', status: 'IMPACT', impactEndedAt: undefined },
      ];
    ...
    test('APOCALYPSE in IMPACT should reduce all resources to 30% (most severe overall)', () => {
      const disasters = [
        { type: 'APOCALYPSE', status: 'IMPACT', impactEndedAt: undefined },
      ];
  ...
  describe('Edge Cases', () => {
    test('should ignore unknown disaster types', () => {
      const disasters = [
        { type: 'UNKNOWN_DISASTER', status: 'IMPACT', impactEndedAt: undefined },
      ];
    ...
    test('should handle missing impactEndedAt in AFTERMATH (treat as just ended)', () => {
      const disasters = [
        { type: 'DROUGHT', status: 'AFTERMATH', impactEndedAt: undefined },
      ];
  ...
  describe('Disaster Type Coverage', () => {
    const disasterTypes = [
      'DROUGHT', 'WILDFIRE', 'FLOOD', 'EARTHQUAKE', 'VOLCANIC_ERUPTION',
      'LANDSLIDE', 'SINKHOLE', 'AVALANCHE', 'HURRICANE', 'TORNADO',
      'TSUNAMI', 'MONSOON', 'SANDSTORM', 'PLAGUE', 'LOCUST_SWARM',
      'BLIGHT', 'BLIZZARD', 'HEATWAVE', 'METEOR_STRIKE', 'COLD_SNAP',
      'SUPERSTORM', 'MEGAQUAKE', 'APOCALYPSE',
    ];

    test.each(disasterTypes)('%s should have production penalties defined', (disasterType) => {
      const disasters = [
        { type: disasterType, status: 'IMPACT', impactEndedAt: undefined },
      ];
