...
describe('Disaster Scheduler', () => {
	describe('World Template Configurations', () => {
		it('should return RARE config for SURVIVAL mode', () => {
			const config = getWorldDisasterConfig('SURVIVAL');
			expect(config.frequency).toBe('RARE');
			expect(config.probabilityPerHour).toBe(0.005);
			expect(config.severityMultiplier).toBe(1.5);
			expect(config.warningTimeMultiplier).toBe(0.75);
		});
		...
		it('should return FREQUENT config for APOCALYPSE mode', () => {
			...
			expect(config.probabilityPerHour).toBe(0.04);
			expect(config.severityMultiplier).toBe(2);
	...
	describe('Disaster Severity Calculation', () => {
		...
		it('should apply template multiplier correctly', () => {
			...
			const severities = Array.from({ length: 100 }, () => calculateDisasterSeverity(highMultiplier));
		...
		it('should apply biome vulnerability correctly', () => {
			...
			const severities = Array.from({ length: 100 }, () => calculateDisasterSeverity(1, vulnerability));
			const avgSeverity = severities.reduce((a, b) => a + b, 0) / severities.length;
	...
	describe('Disaster Type Selection by Biome', () => {
		...
		it('should favor high-risk disasters over moderate and low', () => {
			...
			const highRiskCount = BIOME_DISASTER_MAP.GRASSLAND.highRisk
				.reduce((sum, type) => sum + (selections[type] || 0), 0);
			const moderateRiskCount = BIOME_DISASTER_MAP.GRASSLAND.moderateRisk
				.reduce((sum, type) => sum + (selections[type] || 0), 0);
			const lowRiskCount = BIOME_DISASTER_MAP.GRASSLAND.lowRisk
				.reduce((sum, type) => sum + (selections[type] || 0), 0);
	...
	describe('Regional Disaster Triggering (Integration)', () => {
		...
		it('should not trigger disaster when probability check fails', async () => {
			...
			mockDb.then.mockResolvedValueOnce([mockWorld]); // World query
			mockDb.then.mockResolvedValueOnce([{ biome: 'GRASSLAND' }]); // Tile query
		...
		it('should trigger disaster when probability check succeeds', async () => {
			...
			mockDb.then.mockResolvedValueOnce([mockWorld]); // World query
			mockDb.then.mockResolvedValueOnce([{ biome: 'GRASSLAND' }]); // Tile query
			mockDb.then.mockResolvedValueOnce({}); // Insert query
		...
		it('should return null if world not found', async () => {
			mockDb.then.mockResolvedValueOnce([]); // Empty world query
	...
	describe('Constants and Type Safety', () => {
		it('should export all disaster types', () => {
			expect(ALL_DISASTER_TYPES.length).toBe(16); // Schema has 16 types
		});
